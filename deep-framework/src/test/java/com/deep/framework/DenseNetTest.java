...
public class DenseNetTest extends Shape {
    ...
    public void DenseNetTest3() {
        ...
        Tensor tensor11 = tf.convx(1, 1, new Tensor("weight", new int[]{10, 3, 3}), input);//16*140*140
        Tensor tensor12 = tf.relux(tensor11);//16*140*140
        Tensor tensor13 = tf.convx(1, 1, new Tensor("weight", new int[]{10, 3, 3}), tensor12);//16*140*140
        Tensor tensor14 = tf.relux(tensor13);//16*140*140
        Tensor tensor15 = tf.maxpoolx(2, 2, 0, tensor14);//16*70*70

        Tensor tensor21 = tf.convx(1, 1, new Tensor("weight", new int[]{20, 3, 3}), tensor15);//20*70*70
        Tensor tensor22 = tf.relux(tensor21);//20*70*70
        Tensor tensor23 = tf.convx(1, 1, new Tensor("weight", new int[]{20, 3, 3}), tensor22);//20*70*70
        Tensor tensor24 = tf.relux(tensor23);//20*70*70
        Tensor tensor25 = tf.maxpoolx(2, 2, 0, tensor24);//20*35*35

        Tensor tensor31 = tf.convx(1, 1, new Tensor("weight", new int[]{40, 3, 3}), tensor25);//32*35*35
        Tensor tensor32 = tf.relux(tensor31);//32*35*35

        Tensor tensor41 = tf.demaxpoolx(2, 2, 0, tensor32);//32*70*70
        Tensor tensor42 = tf.convx(1, 1, new Tensor("weight", new int[]{20, 3, 3}), tensor41);//20*70*70
        Tensor tensor43 = tf.relux(tensor42);//20*70*70
        Tensor tensor44 = tf.convx(1, 1, new Tensor("weight", new int[]{20, 3, 3}), tensor43);//20*70*70
        Tensor tensor45 = tf.relux(tensor44);//20*70*70

        Tensor tensor51 = tf.demaxpoolx(2, 2, 0, tensor45);//20*140*140
        Tensor tensor52 = tf.convx(1, 1, new Tensor("weight", new int[]{10, 3, 3}), tensor51);//16*140*140
        Tensor tensor53 = tf.relux(tensor52);//16*140*140
        Tensor tensor54 = tf.convx(1, 1, new Tensor("weight", new int[]{10, 3, 3}), tensor53);//16*140*140
        Tensor tensor55 = tf.relux(tensor54);//16*69*69

        Tensor tensor61 = tf.convx(1, 1, new Tensor("weight", new int[]{8, 3, 3}), tensor55);//3*140*140
