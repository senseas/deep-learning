...
public class TransformerTest {
    int batch_size = 1, context_size = 500, header_num = 8, dim = 512;
    ...
    public void TransformerTest() {
        ...
        Tensor tensor21 = tf.multiHeadAttention(dim, header_num, scaler, tensor13);//500*512
        Tensor tensor22 = tf.linear(tensor21, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor23 = tf.addx(tensor21, tensor22);//500*512
        Tensor tensor24 = tf.layerNormal(tensor23);//500*512

        //MultiHeadAttention & Linear Add & Normal
        Tensor tensor31 = tf.multiHeadAttention(dim, header_num, scaler, tensor24);//500*512
        Tensor tensor32 = tf.linear(tensor31, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor33 = tf.addx(tensor31, tensor32);//500*512
        Tensor tensor34 = tf.layerNormal(tensor33);//500*512

        //MultiHeadAttention & Linear Add & Normal
        Tensor tensor41 = tf.multiHeadAttention(dim, header_num, scaler, tensor34);//500*512
        Tensor tensor42 = tf.linear(tensor41, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor43 = tf.addx(tensor41, tensor42);//500*512
        Tensor tensor44 = tf.layerNormal(tensor43);//500*512

        //MultiHeadAttention & Linear Add & Normal
        Tensor tensor51 = tf.multiHeadAttention(dim, header_num, scaler, tensor44);//500*512
        Tensor tensor52 = tf.linear(tensor51, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor53 = tf.addx(tensor51, tensor52);//500*512
        Tensor tensor54 = tf.layerNormal(tensor53);//500*512

        //MultiHeadAttention & Linear Add & Normal
        Tensor tensor61 = tf.multiHeadAttention(dim, header_num, scaler, tensor54);//500*512
        Tensor tensor62 = tf.linear(tensor61, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor63 = tf.addx(tensor61, tensor62);//500*512
        Tensor tensor64 = tf.layerNormal(tensor63);//500*512

        //MultiHeadAttention & Linear Add & Normal
        Tensor tensor71 = tf.multiHeadAttention(dim, header_num, scaler, tensor64);//500*512
        Tensor tensor72 = tf.linear(tensor71, new Tensor(new int[]{dim, dim}));//500*512
        Tensor tensor73 = tf.addx(tensor71, tensor72);//500*512
        Tensor tensor74 = tf.layerNormal(tensor73);//500*512

        //Linear
        Tensor tensor55 = tf.linear(tensor74, new Tensor(new int[]{dim, num}));//500*61224
        Tensor tensor56 = tf.linear(new Tensor(new int[]{1, context_size}), tensor55);//1*61224
    ...
    public List<String> getInput(List<String> words, int index) {
        ...
        IntStream.range(data.size(), context_size).forEach(i -> data.add("<pad>"));
        return data;
    }
