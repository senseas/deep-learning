...
public class LeNetTest extends Shape {
    ...
    public void LeNetTest() {
        ...
        Tensor tensor11 = tf.convx(1, 0, new Tensor("weight", new int[]{10, 5, 5}), input);//6*24
        Tensor tensor12 = tf.relux(tensor11);//6*24
        Tensor tensor13 = tf.maxpoolx(2, 2, 0, tensor12);//6*12

        Tensor tensor21 = tf.convx(1, 0, new Tensor("weight", new int[]{16, 5, 5}), tensor13);//16*8
        Tensor tensor22 = tf.relux(tensor21);//16*8
        Tensor tensor23 = tf.maxpoolx(2, 2, 0, tensor22);//16*4

        Tensor tensor31 = tf.convx(1, 0, new Tensor("weight", new int[]{32, 4, 4}), tensor23);//32*1
